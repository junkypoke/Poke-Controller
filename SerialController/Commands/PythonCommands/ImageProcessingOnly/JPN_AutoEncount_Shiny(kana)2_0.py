...
class AutoEncount(ImageProcPythonCommand):
    NAME = 'JPN(かな)ランダム・シンボル色厳選 ver.2.0'
    ...
    def do(self):
        print("-------------------------------")
        print("JPN(かな)ランダム・シンボル色厳選 ver.2.0")
        ...
        while True:
            ...
            if self.isContainTemplate('Network_Offline.png',0.7, show_value=False):
                print("Offline_Check")
                while not (self.isContainTemplate('yasei_kana.png', threshold=0.9, use_gray=True, show_value=False) \
                    or self.isContainTemplate('yasei2_kana.png', threshold=0.9, use_gray=True, show_value=False)):
