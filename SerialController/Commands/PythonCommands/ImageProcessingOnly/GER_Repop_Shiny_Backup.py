...
class AutoEncount(ImageProcPythonCommand):
    ...
    def do(self):
        print("-------------------------------")
        print("GER_BU_リポップ色違い厳選 ver.1.0")
        ...
        while True:
            ...
            while not self.isContainTemplate('yasei_GER.png', threshold=0.8, use_gray=True, show_value=False):
                self.wait(0.1)
                count2 += 1
                if int(count2) % 20 == 0:
                    self.press(Button.LCLICK, 0.2)
                    ...
                    break
            # エンカウント処理
            if self.isContainTemplate('yasei_GER.png',0.8, use_gray=False, show_value=False):
                print("判定中...")
